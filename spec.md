...
5. Выдавать новую клиентскую конфигурацию, QR-код и совместимую ссылку `vpn://` ровно один раз.
...
5. Администратор создаёт новое подключение, сразу показывает или скачивает `.conf`, показывает QR-код либо копирует одноразовую ссылку `vpn://` для вставки в AmneziaVPN.
...
6. Web локально строит QR-код, предлагает скачать `.conf` и формирует совместимую с AmneziaVPN/DefaultVPN ссылку `vpn://` из того же одноразового payload. Ссылка доступна для явного копирования, не используется как web-навигация, не отправляется отдельным API-запросом и удаляется из состояния Web вместе с исходной конфигурацией после закрытия окна.
...
Пара `(instanceId, addressCidr)` уникальна для всех подключений, кроме `revoked`:
отозванный peer уже отсутствует в конфигурации Instance, поэтому его адрес может
быть безопасно выдан заново. `suspended`, `expired`, `quota-exceeded` и `error`
продолжают резервировать адрес, поскольку peer может быть восстановлен или требует
явного разрешения состояния.
...
- Миграция повторного использования адресов сохраняет все Connection и связанные
  traffic rollups, заменяет глобальную уникальность `(instanceId, addressCidr)` на
  частичную уникальность для статусов, отличных от `revoked`, и проходит
  `foreign_key_check`. Перед ней также создаётся SQLite backup.
